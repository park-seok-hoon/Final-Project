...
<head>
    ...
    <link rel="stylesheet" href="<c:url value ="/resources/css/product_update_list.css"/>">
        ...
        <div class="right_contents">
            ...
            <div class="sub_contents">
                <div class="product_title">
                    <p>
                        <span>스메그 상품</span>
                        <span>수정할 제품이름을 클릭해주세요.</span>
                    </p>
                    <!-- 폼태그를 추가해서 비동기가 아닌 동기적으로 그룹 선택할때마다 페이지가 변경되게 처리했습니다. -->
                    <form action="<c:url value="/admin/update_list"/>" method="get" id="group-form">
							...
							<c:if test="${productlist.size() > 0 }">
								<c:forEach items="${productlist}" var="product">
									<tr>
				                        ...
				                        <td>
				                        	<!-- 컨트롤러가 아닌 주소 url의 파라미터로 그룹명과 카테고리명 보내서 update.jsp가 바로 써서 조인 없이 사용가능하게 처리 -->
				                        	<c:url value="/admin/update" var="url">
												<c:param name="prCode" value="${product.pr_code}"/>
												<c:param name="grName" value="${product.pr_gr_name}"/>
												<c:param name="caName" value="${product.pr_ca_name}"/>
											</c:url>
											<a href="${url}">
				                        		${product.pr_name}
				                        	</a>
	            ...
	            <ul class="pagination">
					<c:if test="${pm.prev}">
						...
						<!-- 이전버튼 -->
						<li>
							<c:url var="url" value="/admin/update_list">
		        ...
		        <!-- 검색처리 -->
				<form action="<c:url value="/admin/update_list"/>" method="get">
